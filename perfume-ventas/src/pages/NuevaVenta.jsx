...
import { Search, Plus, Minus, Trash2, ShoppingBag, Tag, ChevronDown, ChevronUp, X } from 'lucide-react'
...
export default function NuevaVenta() {
  ...
  // Descuento global: se reparte proporcional al precio de cada item
  function aplicarDescuentoGlobal(valorStr) {
  ...
  // Panel del carrito (compartido entre móvil y desktop)
  const CarritoPanel = () => (
            ...
            <div key={item.producto.id} style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <button onClick={() => cambiarCantidad(item.producto.id, -1)} style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', borderRadius: '4px', color: 'var(--text-primary)', width: '22px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Minus size={10} />
                  </button>
              ...
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'none', color: item.descuento > 0 ? 'var(--success)' : 'var(--text-muted)', fontSize: '11px', padding: '2px 0' }}
              >
                <Tag size={10} />
              ...
              {editandoDescuento === item.producto.id && (
                <div style={{ marginTop: '7px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                  <div>
                    ...
                    <input type="number" value={item.descuento || ''} onChange={e => aplicarDescuento(item.producto.id, e.target.value)} placeholder="0.00" min="0" style={{ padding: '5px 8px', fontSize: '12px' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: '10px', color: 'var(--text-muted)', display: 'block', marginBottom: '3px' }}>Precio final S/</label>
                    <input type="number" value={item.precioFinal || ''} onChange={e => setPrecioManual(item.producto.id, e.target.value)} min="0" style={{ padding: '5px 8px', fontSize: '12px' }} />
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={{ padding: '14px 16px', borderTop: '1px solid var(--border)' }}>
            ...
            <div style={{ marginBottom: '12px', background: 'var(--surface-raised)', borderRadius: 'var(--radius)', padding: '10px 12px', border: '1px solid var(--border)' }}>
              ...
              {descuentoGlobal && parseFloat(descuentoGlobal) > 0 && (
                <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '5px' }}>
                  Se divide proporcionalmente entre {carrito.length} producto{carrito.length > 1 ? 's' : ''}
                </div>
              )}
            </div>
          )}
          ...
          <div style={{ marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '5px' }}>
              {METODOS_PAGO.map(m => (
                <button key={m} onClick={() => setMetodoPago(m)} style={{ flex: 1, padding: '7px 4px', fontSize: '11px', fontWeight: 500, borderRadius: 'var(--radius-sm)', background: metodoPago === m ? 'var(--gold)' : 'var(--surface-raised)', color: metodoPago === m ? 'var(--obsidian)' : 'var(--text-secondary)', border: `1px solid ${metodoPago === m ? 'var(--gold)' : 'var(--border)'}` }}>
                  {m}
                </button>
              ))}
            </div>
          ...
          <textarea placeholder="Notas (opcional)..." value={notas} onChange={e => setNotas(e.target.value)} rows={2} style={{ fontSize: '12px', marginBottom: '10px', resize: 'none' }} />

          <button onClick={registrarVenta} disabled={guardando} style={{ width: '100%', padding: '13px', background: guardando ? 'var(--gold-dim)' : 'var(--gold)', color: 'var(--obsidian)', borderRadius: 'var(--radius)', fontWeight: 700, fontSize: '14px', letterSpacing: '0.03em' }}>
  ...
  return (
    <>
      ...
      {/* MÓVIL: una columna + botón flotante */}
      <div className="mobile-layout" style={{ display: 'none', maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '5px', marginBottom: '12px', flexWrap: 'wrap' }}>
          {categorias.map(cat => (
            <button key={cat} onClick={() => setCategoriaFiltro(cat)} style={{ padding: '6px 12px', borderRadius: 'var(--radius-sm)', fontSize: '12px', fontWeight: 500, background: categoriaFiltro === cat ? 'var(--gold)' : 'var(--surface)', color: categoriaFiltro === cat ? 'var(--obsidian)' : 'var(--text-secondary)', border: '1px solid var(--border)' }}>
              {cat}
            </button>
          ))}
        </div>
          ...
          {productosFiltrados.map(producto => {
            ...
            return (
              <button key={producto.id} onClick={() => agregarAlCarrito(producto)} style={{ background: enCarrito ? 'var(--gold-glow)' : 'var(--surface)', border: `1px solid ${enCarrito ? 'var(--gold-dim)' : 'var(--border)'}`, borderRadius: 'var(--radius)', padding: '12px', textAlign: 'left', cursor: 'pointer', position: 'relative' }}>
                ...
                {enCarrito && (
                  <div style={{ position: 'absolute', top: '6px', right: '6px', width: '18px', height: '18px', background: 'var(--gold)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 700, color: 'var(--obsidian)' }}>
                    {enCarrito.cantidad}
                  </div>
                )}
              </button>
        ...
        {carrito.length > 0 && !mostrarCarrito && (
          ...
          >
            ...
            <span style={{ background: 'var(--obsidian)', color: 'var(--gold)', borderRadius: '12px', padding: '1px 7px', fontSize: '11px' }}>
              {totalItems}
            </span>
          </button>
        )}
